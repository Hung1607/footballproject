//package nl.fontys.individualproject.repositories;
//
//import jakarta.persistence.EntityManager;
//import jakarta.transaction.Transactional;
//import nl.fontys.individualproject.domain.enume.PlayingLocation;
//import nl.fontys.individualproject.domain.enume.Position;
//import nl.fontys.individualproject.repositories.Entity.PlayerEntity;
//import org.junit.jupiter.api.AfterEach;
//import org.junit.jupiter.api.BeforeEach;
//import org.junit.jupiter.api.Test;
//import org.junit.jupiter.api.extension.ExtendWith;
//import org.springframework.beans.factory.annotation.Autowired;
//import org.springframework.boot.test.autoconfigure.jdbc.AutoConfigureTestDatabase;
//import org.springframework.boot.test.autoconfigure.orm.jpa.DataJpaTest;
//import org.springframework.test.context.junit.jupiter.SpringExtension;
//
//import java.time.LocalDate;
//
//import static org.junit.jupiter.api.Assertions.*;
//
//@Transactional
//@DataJpaTest
//@ExtendWith(SpringExtension.class)
//@AutoConfigureTestDatabase(replace = AutoConfigureTestDatabase.Replace.NONE)
//class PlayerRepositoryTest {
//
//    @Autowired
//    private EntityManager entityManger;
//    @Autowired
//    private PlayerRepository playerRepository;
//    private PlayerEntity playerEntity;
//
//    @BeforeEach
//    void setUp() {
//        playerEntity = PlayerEntity.builder()
//                .username("Jakacc")
//                .gmail("jackie@gmail.com")
//                .password("lombok")
//                .dateOfBirth(LocalDate.of(2002,9,9))
//                .playingLocation(PlayingLocation.EINDHOVEN)
//                .position(Position.FORWARD)
//                .build();
//        entityManger.persist(playerEntity);
//        entityManger.flush();
//    }
//
//    @AfterEach
//    void tearDown() {
//        entityManger.clear();
//    }
//
//    @Test
//    void updatePlayer() {
//        PlayerEntity expected = PlayerEntity.builder()
//                .id(playerEntity.getId())
//                .username("Jakacccocm")
//                .gmail("jackie123@gmail.com")
//                .dateOfBirth(LocalDate.of(2002,9,9))
//                .playingLocation(PlayingLocation.AMSTERDAM)
//                .position((Position.MIDFIELDER))
//                .build();
//
//        this.playerRepository.updatePlayer(playerEntity.getId(),
//                expected.getGmail(),
//                expected.getUsername(),
//                expected.getDateOfBirth(),
//                expected.getPlayingLocation(),
//                expected.getPosition());
//
//        entityManger.flush();
//        entityManger.clear();
//
//        PlayerEntity actual = entityManger.find(PlayerEntity.class,playerEntity.getId());
//
//        assertEquals(actual.getId(),expected.getId());
//        assertEquals(actual.getGmail(),expected.getGmail());
//        assertEquals(actual.getUsername(),expected.getUsername());
//        assertEquals(actual.getPlayingLocation(),expected.getPlayingLocation());
//        assertEquals(actual.getPosition(),expected.getPosition());
//    }
//
//    @Test
//    void updatePlayer_IdDoesNotExist() {
//        PlayerEntity expected = PlayerEntity.builder()
//                .id(123123)
//                .username("Jakacccocm")
//                .gmail("jackie123@gmail.com")
//                .dateOfBirth(LocalDate.of(2002,9,9))
//                .playingLocation(PlayingLocation.AMSTERDAM)
//                .position((Position.MIDFIELDER))
//                .build();
//
//        this.playerRepository.updatePlayer(expected.getId(),
//                expected.getGmail(),
//                expected.getUsername(),
//                expected.getDateOfBirth(),
//                expected.getPlayingLocation(),
//                expected.getPosition());
//
//        PlayerEntity actual = entityManger.find(PlayerEntity.class,playerEntity.getId());
//
//        assertNotEquals(actual.getId(),expected.getId());
//        assertNotEquals(actual.getGmail(),expected.getGmail());
//        assertNotEquals(actual.getUsername(),expected.getUsername());
//        assertNotEquals(actual.getPlayingLocation(),expected.getPlayingLocation());
//        assertNotEquals(actual.getPosition(),expected.getPosition());
//    }
//}